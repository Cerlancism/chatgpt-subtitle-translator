...
Options:
  ...
  - `-b, --batch-sizes <sizes>`
    ...
    When omitted, batch size is determined automatically per batch based on the `--context` token budget, spread evenly across the remaining lines. On failure, the size is reduced and retried; once it reaches the minimum, translation falls back to single-entry mode. After a reduction, the size eases back up stepwise following consecutive successful batches.
